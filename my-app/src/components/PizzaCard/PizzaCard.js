...
const PizzaCard = ({data}) => {
    ...
    return (
        <div className={style.pizzaWrapper}>
            {data.map((pizza) => (
                <motion.div key={pizza.id.toString()} className={style.pizzaCartWrapper} whileHover={{scale: 1.05}} transition={{duration: .7}}>
                    {/* не удалять AnimatePresence */}
                    <AnimatePresence exitBeforeEnter> 
                    <PizzaCartInner name={pizza.name} price={pizza.price} img={pizza.image} id={pizza.id} openModal={openModal}/>
                    </AnimatePresence>
                </motion.div>
            ))}
             <AnimatePresence>
                {modalOpen && <PizzaCardModal modalId={modalId} closeModal={closeModal}/>}
             </AnimatePresence>
        </div>
    )
}
