...
const PizzasMainWrapper = () => {
  ...
  const sortAllPizzaData = () => {
    if (categoryState === 'all')  {
      let newData = data.filter(({price, name}) => {
        return price < +maxPizzaPriceState && name.toLowerCase().includes(pizzaNameState.toLowerCase())
      })
      return sortBySelectSort(newData)   
  }
    
    let newData = data.filter((pizza) => {
      return (pizza[categoryState]) && pizza.price < +maxPizzaPriceState && pizza.name.toLowerCase().includes(pizzaNameState.toLowerCase())
    })
  ...
  return (
    <div className={style.pizzasMainWrapper}>
      ...
      {pizzaAllData.length > pizzaOnCurrentPage &&  <Pagination pizzaOnCurrentPage={pizzaOnCurrentPage} totalPizza={pizzaAllData.length} changeCurrentPage={changeCurrentPage} currentPage={currentPage}/>}
    </div>
  );
}
